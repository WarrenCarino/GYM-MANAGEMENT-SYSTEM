...
function TrainerLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    if (!contactNumber || !password) {
      setError("Please provide both contact number and password");
      return;
    }
    ...
    try {
      ...
      const res = await axios.post("/api/authtrainers/login", payload, {
        headers: {
      ...
      // Store login information like cashier app
      localStorage.setItem("contactNumber", contactNumber);
      console.log("✅ Contact number stored in localStorage:", contactNumber);

      if (user.id) {
        localStorage.setItem("trainerId", user.id);
        console.log("✅ Trainer ID stored in localStorage:", user.id);
      }
      if (user.name) {
        localStorage.setItem("trainerName", user.name);
        console.log("✅ Trainer name stored in localStorage:", user.name);
      }
      if (user.email) {
        localStorage.setItem("trainerEmail", user.email);
        console.log("✅ Trainer email stored in localStorage:", user.email);
      }

      // Store member contact if available
      ...
      // Log error login to audit trail
      try {
        await axios.post("/api/audit-trail", {
          id: contactNumber,
          role: "trainer",
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          🔒 Secure Trainer Access | Encrypted Connection
        </div>
      </div>
    </div>
  );
